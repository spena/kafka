...
public class JoinedValuesSerializer<V1, V2> implements WrappingNullableSerializer<JoinedValues<V1, V2>, Void, JoinedValues<V1, V2>> {
    private final Serializer<V1> thisSerializer;
    private final Serializer<V2> otherSerializer;

    public JoinedValuesSerializer(final Serializer<V1> thisSerializer, final Serializer<V2> otherSerializer) {
        this.thisSerializer = Objects.requireNonNull(thisSerializer);
        this.otherSerializer = Objects.requireNonNull(otherSerializer);
    }

    @Override
    public byte[] serialize(final String topic, final JoinedValues<V1, V2> data) {
        if (data == null) {
            return null;
        }
    ...
    @Override
    public void setIfUnset(final Serializer<Void> defaultKeySerializer, final Serializer<JoinedValues<V1, V2>> defaultValueSerializer) {
